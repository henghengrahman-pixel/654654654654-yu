...
export function pollerStatus(){ return {
  ...
  traffic:{polled:false,processing:false,source:'LiveChat Engage Traffic UI',reason:'Browsing traffic is intentionally excluded from AI chat processing'},
...
async function processChatSnapshot(livechat,chat,summary=null,rank=null,{force=false}={}){
  ...
  if(!events.length){ await db.clearBootstrapped(chatId); forgetChat(chatId); await db.logError('poller','EMPTY_CHAT_DETAIL','LiveChat get_chat returned no readable message events',{chatId,diagnostics:livechat.chatDiagnostics(detail)}); return {empty:1}; }
...
async function runDeepSync(livechat){
  ...
  try{
    ...
    await db.setIntegrationHealth('livechat_deep_sync',{status:'OK',latencyMs:Date.now()-started,meta:{activeChats:chats.length,myChats:counts.MY_CHAT,queued:counts.QUEUED,supervised:counts.SUPERVISED,pages:all.pages,foundChats:all.foundChats}}).catch(()=>{});
    return {ok:true,activeChats:chats.length,lanes:counts,pages:all.pages};
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try{
    ...
    let queued=0,unchanged=0,enqueueErrors=0,lightweight=0;
    const counts={MY_CHAT:0,QUEUED:0,SUPERVISED:0,CLOSED:0,OTHER:0};
    ...
    for (const [rank,summary] of chats.entries()){
      ...
      try{
        ...
        if(oldFp===fp && oldLane===lane && oldRank===rank){unchanged++;continue;}
        ...
        if(lane==='MY_CHAT'){
          const job=await db.enqueueLiveChatIngressJob({dedupeKey:`poll:${chatId}:${fp}`,jobType:'SYNC_CHAT',chatId,payload:{summary},priority:90});
    ...
    lastResult={ok:true,listSource:data?._listSource||'unknown',rawChats:rawChats.length,chats:chats.length,myChats:counts.MY_CHAT,queuedLane:counts.QUEUED,supervised:counts.SUPERVISED,queued,lightweight,unchanged,enqueueErrors,processed:0,newMessages:0,inventorySize,inventoryComplete,deepSyncRunning,concurrency:config.lcSyncConcurrency,durationMs:Date.now()-syncStarted};
    await db.setIntegrationHealth('livechat_poll',{status:'OK',latencyMs:Date.now()-syncStarted,meta:{chats:chats.length,myChats:counts.MY_CHAT,queuedLane:counts.QUEUED,supervised:counts.SUPERVISED,queued,lightweight,unchanged,enqueueErrors}}).catch(()=>{}); return lastResult;
