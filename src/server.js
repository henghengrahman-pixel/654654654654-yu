...
app.get('/api/conversations',requireAdmin,async(req,res)=>{
  ...
  const requestedLane=String(req.query.lane||'ALL').trim().toUpperCase();
  const lane=['ALL','MY_CHAT','QUEUED','SUPERVISED'].includes(requestedLane)?requestedLane:'ALL';
  let cursor=null;
  ...
  const [r,countRows]=await Promise.all([
    ...
    LIMIT $3`,params),
    pool.query(`SELECT lc_lane,count(*)::int AS count FROM conversations WHERE visible_in_inbox=true AND status='active' AND lc_active IS DISTINCT FROM false GROUP BY lc_lane`)
  ]);
  ...
  const laneCounts={MY_CHAT:0,QUEUED:0,SUPERVISED:0};
  for(const row of countRows.rows||[]) if(Object.prototype.hasOwnProperty.call(laneCounts,String(row.lc_lane||''))) laneCounts[String(row.lc_lane)]=Number(row.count||0);
  res.json({ok:true,items,lane,laneCounts,traffic:{polled:false,url:'https://my.livechatinc.com/engage/traffic'},page:{limit,nextCursor,hasMore}});
});
